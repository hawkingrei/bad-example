...
func TestDiscoverySelf(t *testing.T) {
	Convey("test TestDiscoverySelf ", t, func() {
		...
		So(len(d.node), ShouldNotEqual, 0)
	})
}
		...
		for _, a := range []string{":8080"} {
			go func(addr string) {
				...
				e.GET("/discovery/polls", func(ctx *blademaster.Context) {
					...
					if ts == "0" {
						ctx.JSON(map[string]appData{appdID: {LastTs: time.Now().UnixNano(), ZoneInstances: map[string][]*naming.Instance{"zone": {in}}}}, nil)
					} else {
...
func TestWatchContinuosly(t *testing.T) {
	Convey("test TestWatchContinuosly ", t, func() {
		...
		in1 := *in
		in1.AppID = "test.test"
		cancel, err := d.Register(&in1)
		So(err, ShouldBeNil)
		defer cancel()
		in2 := *in
		in2.AppID = "test.test2"
		cancel2, err := d.Register(&in2)
		So(err, ShouldBeNil)
		defer cancel2()
		in3 := *in
		in3.AppID = "test.test3"
		cancel3, err := d.Register(&in3)
		So(err, ShouldBeNil)
		defer cancel3()
