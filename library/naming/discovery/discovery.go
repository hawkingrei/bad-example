...
func (r *Resolver) Fetch() (ins map[string][]*naming.Instance, ok bool) {
	r.d.mutex.RLock()
...
func (d *Discovery) pickNode() string {
	if len(d.node) == 0 {
		return d.conf.Nodes[rand.Intn(len(d.conf.Nodes))]
	}
	nodes := d.node
