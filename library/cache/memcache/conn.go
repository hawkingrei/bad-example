...
func (c *conn) populateOne(cmd string, item *Item) (err error) {
	...
	if err != nil {
		return c.fatal(pkgerr.WithStack(err))
	}
...
func (c *conn) parseGetReply(f func(*Item)) error {
	...
	for {
		...
		if !bytes.HasSuffix(it.Value, crlf) {
			return pkgerr.WithStack(protocolError("corrupt get reply, no except CRLF"))
		}
...
func scanGetReply(line []byte, item *Item) (size int, err error) {
	if !bytes.HasSuffix(line, crlf) {
		return 0, pkgerr.WithStack(protocolError("corrupt get reply, no except CRLF"))
	}
	// VALUE <key> <flags> <bytes> [<cas unique>]
	chunks := strings.Split(string(line[:len(line)-2]), spaceStr)
	if len(chunks) < 4 {
		return 0, pkgerr.WithStack(protocolError("corrupt get reply"))
	}
	if chunks[0] != replyValueStr {
		return 0, pkgerr.WithStack(protocolError("corrupt get reply, no except VALUE"))
	}
	item.Key = chunks[1]
