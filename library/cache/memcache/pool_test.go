...
func TestPoolGetLargeValue(t *testing.T) {
	...
	if err = conn.Scan(item, &res); err != nil {
		t.Errorf("memcache: large scan error(%v)", err)
	} else if res.Mid != 1000 {
		t.Errorf("memcache: large value mid error(%v)", err)
	}
}
...
func TestPoolGetLargeValueBoundary(t *testing.T) {
	...
	if err := conn.Scan(item, &res); err != nil {
		t.Errorf("memcache: large scan error(%v)", err)
	} else if res.Mid != 1000 || len(res.Value) != _largeValue {
		t.Errorf("memcache: large value len(%v) error(%v)", len(res.Value), err)
	}
}
