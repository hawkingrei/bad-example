...
func testVerifyFailed(t *testing.T) {
	res := Response{}
	code, content, err := httpGet("http://127.0.0.1:18080/verify")
...
func testVerifySuccess(t *testing.T) {
	res := Response{}
	err := client().Get(context.TODO(), "http://127.0.0.1:18080/verify", "", url.Values{}, &res)
	assert.NoError(t, err)
...
func testVerifyUserFailed(t *testing.T) {
	res := Response{}
	code, content, err := httpGet("http://127.0.0.1:18080/verifyUser")
	assert.NoError(t, err)
	assert.Equal(t, 200, code)
	err = json.Unmarshal(content, &res)
	assert.NoError(t, err)
