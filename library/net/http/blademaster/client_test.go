...
func TestClient(t *testing.T) {
	...
	// test Get
	if err := client.Get(context.Background(), "http://api.hhhhhh.com/x/server/now", "", nil, &res); err != nil {
	...
	// test Post
	if err := client.Post(context.Background(), "http://api.hhhhhh.com/x/server/now", "", nil, &res); err != nil {
		t.Errorf("HTTPClient: expected no error but got %v", err)
	}
	if res.Code != -405 {
	...
	client.SetConfig(&ClientConfig{
		Host: map[string]*ClientConfig{"api.hhhhhh.com": {Timeout: xtime.Duration(time.Millisecond * 300)}},
	})
	if err := client.Get(context.Background(), "http://api.hhhhhh.com/x/server/now", "", nil, &res); err != nil {
		t.Errorf("HTTPClient: expected no error but got %v", err)
	}
	client.SetConfig(&ClientConfig{
		Host: map[string]*ClientConfig{"api.hhhhhh.com": {Timeout: xtime.Duration(time.Millisecond * 1)}},
	})
	if err := client.Get(context.Background(), "http://api.hhhhhh.com/x/server/now", "", nil, &res); err == nil {
...
func TestDo(t *testing.T) {
	var (
		aid    = 5463320
		uri    = "http://api.hhhhhh.com/x/server/now"
		req    *http.Request
		client *Client
		err    error
	)
...
func BenchmarkDo(b *testing.B) {
	...
	cf := &ClientConfig{
		...
		URL: map[string]*ClientConfig{
			"http://api.hhhhhh.com/x/server/now":  {Timeout: xtime.Duration(time.Second)},
			"http://api.hhhhhh.com/x/server/nowx": {Timeout: xtime.Duration(time.Second)},
		},
	}
	client := NewClient(cf)
	uri := "http://api.hhhhhh.com/x/server/now"
	...
	uri = "http://api.hhhhhh.com/x/server/nowx" // NOTE: for breaker
	b.ResetTimer()
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			// client.SetConfig(cf)
...
func TestPB(t *testing.T) {
	...
	var res = new(archive.BvcResponseMsg)
