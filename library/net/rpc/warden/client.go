...
func NewClient(conf *ClientConfig, opt ...grpc.DialOption) (c *Client) {
	...
	c.UseOpt(opt...)
	c.Use(c.recovery(), c.handle(), wrr.Stats())
	return
}
...
func (c *Client) Dial(ctx context.Context, target string, opt ...grpc.DialOption) (conn *grpc.ClientConn, err error) {
	...
	if len(c.conf.Clusters) > 0 || c.conf.Zone != "" {
		var (
			u *url.URL
		)
		if u, err = url.Parse(target); err != nil {
			return nil, errors.Errorf("warden: dial target url.Parse(%s) failed!err:=%v", target, err)
		}
		v := u.Query()
