...
func TestReportTCP(t *testing.T) {
	...
	report := newReport("tcp", "127.0.0.1:6077", 0)
	data := "hello, world"
	report.Report(bytes.NewBufferString(data))
	if err := report.Close(); err != nil {
		t.Error(err)
	}
	cancel()
	assert.Equal(t, data, buf.String(), "receive data")
}
...
func TestReportUnixgram(t *testing.T) {
	...
	report := newReport("unixgram", "/tmp/trace.sock", 0)
	data := "hello, world"
	report.Report(bytes.NewBufferString(data))
	if err := report.Close(); err != nil {
		t.Error(err)
