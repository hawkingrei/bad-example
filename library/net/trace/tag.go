...
func String(key string, v string) *proto.Tag {
	return &proto.Tag{Key: key, Kind: proto.Tag_STRING, Value: []byte(v)}
}

// Int new tag Int.
func Int(key string, v int) *proto.Tag {
	return &proto.Tag{Key: key, Kind: proto.Tag_INT, Value: []byte(strconv.Itoa(v))}
}

// Bool new tagBool
func Bool(key string, v bool) *proto.Tag {
	return &proto.Tag{Key: key, Kind: proto.Tag_BOOL, Value: []byte(strconv.FormatBool(v))}
}

// Log new log.
func Log(key string, v string) *proto.Log {
	return &proto.Log{Key: key, Kind: proto.Log_STRING, Value: []byte(v), Timestamp: time.Now().UnixNano()}
}
