...
func (c *Config) filter(d map[string]interface{}) {
	for _, k := range c.Filter {
		d[k] = "***"
	}
}
...
func Init(conf *Config) {
	...
	// when env is not dev
	if conf.Agent != nil || (isNil && env.DeployEnv != "" && env.DeployEnv != env.DeployEnvDev) {
