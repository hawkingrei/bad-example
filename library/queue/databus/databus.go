...
func New(c *Config) *Databus {
	...
	if env.DeployEnv != "" && env.DeployEnv != env.DeployEnvDev {
	...
	for {
		...
		if c == nil || c.Err() != nil {
			if c, err = d.d(); err != nil {
				log.Error("redis.Dial(%s@%s) retry error(%v)", d.conf.Proto, d.conf.Addr, err)
				continue
			}
		}
		d.lock.RLock()
