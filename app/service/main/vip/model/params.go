...
type ArgCreateOrder2 struct {
	Mid         int64  `form:"mid" validate:"required,min=1,gte=1"`
	Month       int16  `form:"months" validate:"required,min=1,gte=1"`
	Platform    string `form:"platform"`
...
// GetVipPanelPlat .
func (av *ArgVipConfig) GetVipPanelPlat() int8 {
	switch {
	case av.PanelType == PanelTypeFriend:
		return PlatVipPriceConfigFriendsGift
	case av.MobiApp == "iphone":
		return PlatVipPriceConfigIOS
	case av.MobiApp == "ipad" && av.Device == "pad":
...
// GetVipPanelPlat .
func (avm *ArgVipConfigMonth) GetVipPanelPlat() int8 {
	return PanelPlat(avm.PanelType, avm.MobiApp, avm.Device)
}

//PanelPlat panel plat.
func PanelPlat(panelType, mobiApp, device string) int8 {
	switch {
	case panelType == PanelTypeFriend:
		return PlatVipPriceConfigFriendsGift
	case mobiApp == "iphone":
		return PlatVipPriceConfigIOS
	case mobiApp == "ipad" && device == "pad":
		return PlatVipPriceConfigIPADHD
	default:
		return PlatVipPriceConfigOther
	}
